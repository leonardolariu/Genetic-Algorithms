...
const int dim = 30;
const double leftMargin = -2.048, rightMargin = 2.048;
...
int generationCount, noChange, bestCandidate;
void geneticAlgorithm (double (*f)(double *)) { //f - fitness function
    initializePopulation();
    evaluatePopulation(f);

    for (int i = 0; i < bitLen; ++i) mutationFactor[i] = bitLen;

    generationCount = noChange = 0;
    ...
    while (noChange < 500) {
        ...
        if (generationCount % 100 == 0) changeMutationFactor();
...
void bestImprovementExploitation (bool *X, double (*f)(double *), bool *bestX, double &bestY) {
    for (int i = 0; i < dim*bitLen; ++i)
    {
        X[i] = !X[i];

        double *decodeX = decode(X);
        double currY = f(decodeX);
        delete []decodeX;
...
double bestImprovement (double (*f)(double *)) {
    double best, bestY;
    bool bestX[dim * bitLen];

    double *decodeCandidate = decode(currPop->candidateSol[bestCandidate]);
    best = bestY = f(decodeCandidate);
    ...
    do {
        bestImprovementExploitation(currPop->candidateSol[bestCandidate], f, bestX, bestY);

        if (bestY < best) best = bestY, memcpy (currPop->candidateSol[bestCandidate], bestX, dim * bitLen);
        else foundBetter = false;
    } while (foundBetter);
...
int main()
{
    ...
    for (int i = 0; i < runs; ++i) {
            geneticAlgorithm(RosenbrockFitness);

            double *decodeCandidate = decode(currPop->candidateSol[bestCandidate]);
            curr = Rosenbrock(decodeCandidate);
            delete []decodeCandidate;
            printf("best after geneticAlgorithm: %f\n", curr);

            curr = bestImprovement(Rosenbrock);
            printf("best after bestImprovement: %f\n\n", curr);
...
void changeMutationFactor() {
    int stop = min(generationCount / 100, bitLen);
    for (int i = 0; i < stop; ++ i) mutationFactor[i] = max(mutationFactor[i] - 1, 0);
}

void mutatePopulation() {
    for (int i = 0; i < popSize; ++i)
        for (int j = 0; j < dim*bitLen; ++j)
            if (rand() / (RAND_MAX + 1.) < mutationProbability * mutationFactor[j%bitLen]) currPop->candidateSol[i][j] = !currPop->candidateSol[i][j];
}
